            ...
            @elseif($activeTab === 'holidays')
                <div class="settings-form-box mb-4 {{ auth()->user()->can('hr-setting-manage') ? '' : 'd-none' }}">
                    ...
                    <form id="holidayForm" method="POST" action="{{ route('hr.settings.holidays.store') }}" class="row g-3 align-items-end">
                        ...
                        <div class="col-md-2"><label class="form-label small fw-semibold">Date *</label><input id="holidayDate" type="date" name="holiday_date" class="progga-form-control" required></div>
...
function editHoliday(i){ editBase('holiday',routeBases.holidays,i); holidayName.value=i.name; holidayDate.value=i.date; holidayType.value=i.type; holidayPaid.checked=!!i.paid; holidayDescription.value=i.description||''; holidayStatus.checked=!!i.status; }
function resetHolidayForm(){ resetBase('holiday',@json(route('hr.settings.holidays.store')),'Holiday'); holidayPaid.checked=true; holidayStatus.checked=true; }
