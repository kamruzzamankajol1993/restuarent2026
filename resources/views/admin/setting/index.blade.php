            ...
            <div class="progga-card-body">
                <form action="{{ route('settings.pos') }}" method="POST">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><div class="progga-form-group"><label class="progga-form-label">Final Payment Depends on Kitchen Ready Status</label><label class="progga-toggle" style="margin-top:8px;"><input type="checkbox" name="final_payment_depends_on_kitchen_status" {{ ($pos->final_payment_depends_on_kitchen_status ?? false) ? 'checked' : '' }} data-on="Yes" data-off="No"><span class="progga-toggle-track"><span class="progga-toggle-thumb"></span></span><span class="progga-toggle-label">{{ ($pos->final_payment_depends_on_kitchen_status ?? false) ? 'Yes—Ready required' : 'No—Pay anytime' }}</span></label><small class="text-muted d-block mt-1">Yes হলে Kitchen status Ready না হওয়া পর্যন্ত POS offcanvas payment button disabled থাকবে। No হলে payment করা যাবে এবং final payment এর সময় KOT Delivered হবে।</small></div></div>
